...
## 6. 工具升級：頁面日期 ＋ 短評（SK 2026-09-21 指示）

`mcmod.py` 已加兩樣（live 副本＋skill 包＋`Documents\Code_Project\mcmod-reader` 已同步）：
- `read` 多一行 **`頁面日期：创建日期／最后编辑／編輯次數`**（由 `data-original-title` tooltip 抽到秒）
- 新指令 **`comments <id> [--channel 1|2|3] [--page N]`**：`POST /frame/comment/CommentRow/`（JSON，含 `time.source` 真日期）→ 樓層／用戶／日期／內容。**唔准打 `/action/doComment/`（寫入端點）**。

### 為何重要（實測證據）
| 頁 | 頁面日期 | 短評（較新） |
|---|---|---|
| [591072 風暴之星](https://www.mcmod.cn/item/591072.html) | 创建 2022-05-14／**最后编辑 2025-03-01** | **2025-07-15**：「注意：**新版凋灵风暴的风暴之星已更名为疫染下界之星**」← 正文完全冇、評論先有（＝版本關鍵） |
| [414814 儀式火盆](https://www.mcmod.cn/item/414814.html) | — | 4 條全部係**用法技巧**（魔源罐 6 格範圍、紙箱搬運），**冇**取得途徑更正 |

⇒ **本 pack（1.19.2、CWSM 3.1.1.1）用嘅係「风暴之星」舊名**，同評論講嘅新版更名一致（＝我哋唔應該跟 wiki 新名）。

### C9 QA 流程（更新版；開發期用，唔入 code）
1. `read <id>` → 睇 **頁面日期**（太舊＝要加倍交叉核）
2. `comments <id>`（起碼 channel 1）→ 睇有冇**更名／機制更正／版本差異**
3. `post <教程 id>` → 教程往往有頁面正文冇嘅細節
4. 任何具體 id／名稱／機制 → **必須**用 jar（version-exact）／官方 repo／wiki 交叉；**jar > 官方 > mcmod > 真機**
5. 發現我哋事實有缺 → 只寫入 plan 備忘（**唔准貼 mcmod 原文入 code**，版權）
