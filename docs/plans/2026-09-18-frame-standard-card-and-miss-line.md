# Plan：標準框架＝出返合成卡 ＋ 清走殘留否定句（v3）

- 日期：2026-09-18（**v3 = R1 反方 review 後重寫**；v1／v2 可還原，見檔尾）
- 觸發：SK 真機 3 問（09-18 17:26／17:27／18:22）＋SK 指出「but no cards...」＋SK 揀 **option 1**（一個細 plan 一次過修 1＋2＋3）
- 狀態：**待 R2 反方 review（gate 正方 ≥8 : 反方 ≤2）**；未開工、未改任何 code／jar
- 範圍：**只改 forge/1.19.2**；`neoforge/1.21.1` 唔郁
- Review 記錄：**R1 反方（2026-09-18）＝ 正方 3 : 反方 7**（報告 `docs/plans/reviews/2026-09-18_frame-standard-card-R1-opposing.txt`，26,173 bytes）→ v3 逐條回應見 **§8**（3 條 MUST-FIX 全部接受、0 條駁回）
- 舊版本還原：v2 md5 `376c733a6f4846a09673aaa47d20429c`（＝ commit `8974d9f`）／v1 md5 `f3fd06493ea21375eae03ae728dd86ed`（＝ commit `533f96d`，已實測 `git show 533f96d:… | md5sum` 對得上）
...
| G6 | **（v2 假 claim，已撤回）** v2 寫「`:631` 同 suppress 呼叫點喺**同一個 method** 內（已核）⇒ 唔需要改 `beginAskLoop` 簽名」 | **事實錯**：`:631` 喺 `beginAskLoop`（`:614–649`，由 `:299`／`:2416` 呼叫），`:179` 喺 `runAsk`（`:128–461`）、`:2324` 喺第二個 ask method（`:2276–2509`）。**v3 明寫落地方案 (b)**：`beginAskLoop` 簽名唔改，改喺 `:299`／`:2416` **返回後即場 `askLoop.setFrameStandardKeep(...)`**（public setter 先例：`AskLoopState:526–528`） |
...
| G10（R1 新增） | `logic/AskJeiHints.java:110–139 scrubAbsenceClaimsWhenCards`（由 `AskResult.withRecipeCards` → `:84–97` 觸發） | 今日 `hasCards=false` 唔觸發；**D1 之後 STANDARD 有卡 ⇒ 會開始刪 `looksLikeAbsenceClaim` 行**（既有行為，非本 plan 新增）→ §2 D4 明寫、S6 抽查。R1 已查證：`looksLikeAbsenceClaim:39–78` **唔覆蓋**目標 miss 句 ⇒ D4 唔係重複施工 |
...
3. **tool 卡閘 allow-list**：`AskToolEnv` 新增兩個欄位（`public String frameStandardKeepOutputId = "";`／`public List<String> frameStandardKeepInputIds = List.of();`）；`rejectFrameCard` 改簽名為 `private boolean rejectFrameCard(RecipeCard card)`：
   - 第一句：`if (ModularFrameCards.isStandardKeepCard(card, frameStandardKeepOutputId, frameStandardKeepInputIds)) return false;`（＝放行）
...
### D2 唯一放行點喺 **cap 之前**（解 R1 CRITICAL #1／HIGH #4）

- `logic/RenderRecipeCardsAskTool.java`（**v3 加入白名單**）：喺 `if (matched.size() > PER_CALL_CAP)`（**真行號 `:134`**）**之前**插入：
  `matched = ModularFrameCards.keepOnlyStandardRecipeCard(matched, env == null ? "" : env.frameStandardKeepOutputId, env == null ? List.of() : env.frameStandardKeepInputIds);`
  ⇒ 命中嗰張係**唯一**候選，**唔受 `subList(0, PER_CALL_CAP)` 影響**（「oak 排第 10 就一張都冇」嘅風險消失）。
- `AskTrace.renderCards(... total ...)` 同逐張 `AskTrace.card("check.cards", …)` 維持記錄**過濾前**清單（診斷資訊不變）。
- **匹配規則（純函式，住喺 `ModularFrameCards`）**：
  - `card.primaryOutputId()`（大小寫不敏感）== `keepOutputId`；**並且** `card.inputs()`（`RecipeCard` 真欄位 `List<ItemStack>`）映射出嘅 id 集合 ⊇ `keepInputIds`。
  - **fallback（決定性）**：冇精確命中 → 取 `matched` 內**第一張** `primaryOutputId == keepOutputId` 嘅卡（任何材料變體都係合法框架配方）＋ log `frame-standard: keep-fallback variant=<id> inputs=<n>`；連一張都冇 → 出 0 張 ＋ log `frame-standard: keep-miss matched=<n>`（**唔准靜默**）。
  - 兩個 log 都係 ASCII、唔含玩家文字；`keepOutputId` 空 → 函式原樣回傳（＝今日行為）。
- 「同族 10 變體／mirror 卡」＝ 被呢個函式**擋喺 offer 之前**（唔再靠 dropId 逐張 drop），文字已用「同族材料版本共 10 種」交代（R1 內部矛盾 #3 已解）。
...
1. `forge/1.19.2/src/main/java/com/skps9/packai/logic/ModularFrameCards.java`（新增 `isStandardKeepCard`／`keepOnlyStandardRecipeCard`；`shouldDropFrameCard` 簽名**唔准改**）
...
| jar／部署 | 實作階段唔部署、唔 commit code；**S6 真機驗收例外**（需 build ＋ `mc_mod_deploy_jar.py --jar` 部署，見 §5） |
...
- **S1** harness `ModularFrameCardsCheck`（現 79 行）：`shouldDropFrameCard` 現有 cases 全保留；新增 `isStandardKeepCard`（命中／大小寫／input 缺／keepOut 空）＋`keepOnlyStandardRecipeCard`（精確命中 1 張／fallback 取第一張／全無 → 空）＋**負控**：拿掉 allow-list → 目標卡被 drop（紅）。
...
- **S6（v3 改硬斷言，讀 trace 機械判）**：
  - STANDARD：`render.cards.final.cardsOut == 1` 且該卡 `primaryOutputId == tetra:modular_double`（＝ `recipeResultId`）
  - MODIFIED：`render.cards.final.cardsOut == 0`
  - 命令：`python` 讀 `<instance>/packai/trace/ask-*.jsonl` 抽 `render.cards.final` ＋ `check.cards` 事件；附「今日跑（實作前）會紅」證明（今日 STANDARD 樣本 cardsOut=1 但 `role=input`、`primaryOutputId=mrqx_extra_pack:mystery_craftsmanship`）。
...
- **S8（v3 補命令／欄位，解幽靈閘）**：模型 digest 亦只 1 張框架卡 —— 讀 trace：`send.facts`／卡相關事件內 `tetra:modular_double` 框架卡出現次數 == 1（`check.cards` 事件逐張記錄 `primaryOutputId`；計 `primaryOutputId==tetra:modular_double && role!=input` 嘅張數）。keyword 路徑（`AskCardFallback`）**維持現狀**並記錄（R7）：同一問句用 `cardsMode=keywords` 跑一次，`cardsOut==0` 屬**已知限制**，唔當 regress。
- **S9** 六條 suppress 呼叫點逐條回歸（`:179／:397／:418／:2324／:2479／:2495`）：**新 python 閘 `check_frame_standard_card_keep.py`** 做靜態斷言——① 六個呼叫點全部帶 keep 參數（`grep -n` 命中數 == 6，並列出行號）② `AskToolEnv.rejectFrameCard` body 內**同時**存在 `ModularFrameCards.isStandardKeepCard(` 同 `ModularFrameCards.shouldDropFrameCard(` ③ `RenderRecipeCardsAskTool` 嘅 keep-1 呼叫行號 **<** `PER_CALL_CAP` 截斷行號 ④ `AskEngine.bindAskToolEnv` body 內有 2 個 keep 欄位抄寫 ⑤ `AskLoopState` 有 `setFrameStandardKeep` **負控**：把 ① 嘅其中一條呼叫點嘅 keep 參數拿掉 → 閘必須轉紅（紅→綠證明寫入報告）。
...
- **R2** 匹配靠 id 集合：若 JEI 卡 input id 同 `FrameRecipe.ingredientItemIds` 形狀唔同（tag／流體／NBT 變體）⇒ 走 fallback（出第一張框架卡）＋ log；**唔准**出現「0 張而無 log」。
- **R3** 出卡後會唔會擠走用途卡／超 `MAX_CARD_EMISSIONS`（`logic/AskLoopState.java:73`，＝8）；v3：keep-1 令候選由 20 變 1，反而**降低**壓力；S8／S10 覆蓋。
...
- **R9**（v3 新增）keep 規格係「單一候選」：若 `matched` 內根本冇 `primaryOutputId == keepOutId` 嘅卡（例如 JEI 未收錄該配方類別）⇒ 出 0 張＋`keep-miss` log（**靜默失敗已封**；但玩家仍然冇卡 ⇒ 屬「JEI 冇資料」範疇，另案）。
