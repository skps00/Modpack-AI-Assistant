# Plan v6.14 — 腳本產出通道（診斷 log **逐 site** 一行；S17 用 11 站點；玩家文字零行號）

> 2026-09-17｜v6 線：R1 3:7 → … → v6.11 → R11 **9:1（達標）** →（SK 糾正④）→ v6.12 → R12 4:6（N1–N4）→ v6.13 → **R13 7:3（N1–N4 全 RESOLVED；新 N5／N6）→ v6.14 → R14 進行中**
> 狀態：**R14 正方 8 : 反方 2 → 達標（go=true）**；N5／N6 RESOLVED；唯一殘項 N7（LOW，§2.2 殘句「3」）→ **已即場修**。**等 SK go 派 cursor 實作（單 1）**。
...
2. **Parser** `logic/JsObtainSites.java`：
   ...
   **`rel` 格式（R12 N2 pin）**：`Site.rel` ＝ **相對 instance、含 `kubejs/` 前綴**（例 `kubejs/server_scripts/b_a_d/b_a_d_rclick.js`）——同 fixture `A_hard_gate[*].rel` 一致；**診斷 log 用同一格式，唔准 basename**（10 件 A_hard → 6 unique rel／6 unique basename；basename 唔帶目錄脈絡、跨目錄同名會撞）。只有 `Trigger.dispatcherRel` 用 strip `kubejs/` 嘅短形。
...
| **S17** | **玩家文字零行號＋作者診斷通道（v6.13；收 R12 N1–N4）**：① **禁令＝單一常量** `PLAYER_VISIBLE_FORBIDDEN`＝`(src:)|(\.js\b)|(\.json\b)|(kubejs[/\\])`（plan §0／§2.7／fixture `player_visible_rules.forbidden_regex`／harness **四處字面相同**）；**負斷言**：A_hard 10 件 fact 行＋答案文字零命中；負控：塞 `src:b_a_d_rclick.js:87`、`x.js:12`、`kubejs/server_scripts/a.js` 三款入 fact ⇒ 全部必紅；② **正斷言（作者通道；R13 N5 改為逐 site）**：`JS_OBTAIN_DIAG_LOG=true` → `gameDir/packai/js-obtain-diag.jsonl`（同 `AskTrace.traceDir` 同一 `packai/` root）有 **11 行＝11 個 site**（A_hard 10 件；`ino_dlc_build:pandora_box_blue` sink 7 及 20 ⇒ **兩行**），逐行 `(outId, rel, line)` 同 fixture **`s17_sites`（11 條）** 完全一致（rel 含 `kubejs/` 完整路徑、唔准 basename）、`kind=PRODUCE`；**唔准**用 outId 數（10）當 site 數；③ 開關 false ⇒ 檔案唔存在／零行；④ 兩種開關下玩家可見文字**逐字節相同** | **紅** | harness：1 負斷言（10 件）＋3 負控＋**11** 三元組正斷言（`s17_sites`）＋2 開關斷言 |
...
- **v6 線：R1 3:7 → … → R11 9:1 →（SK 糾正④）→ v6.12 → R12 4:6 → v6.13 → R13 7:3（N1–N4 RESOLVED）→ v6.14 → **R14 正方 8 : 反方 2 → 達標（go=true）**。
- **R14 判定**：N5（`s17_sites`=11、expand(A_hard)=11、pandora 7＋20、S17 明文唔准用 outId 數）／N6（`rel_format.why` 6/6，與實測一致）**RESOLVED**；新 **N7 LOW**＝plan §2.2 L68 殘句仍寫「3 個 unique basename」（fixture／§6 已 6）→ **已即場修**。報告：`docs/plans/reviews/2026-09-16_plan-v6.14-obtain-code-channel-R14-opposing.txt`（今輪 cursor **有**寫檔 ✓）。
- **R13 判定**：N1（fixture 殘句）／N2（`Site.rel` 完整路徑＋diag root）／N3（§3 明文作廢 v6.7 S1 src assert）／N4（`PLAYER_VISIBLE_FORBIDDEN` 單一常量、實測 4 款字串全 match）**全部 RESOLVED**；**N5 HIGH**＝S17 要求 10 件各一行，但 `pandora_box_blue` sink 7 及 20 ⇒ 真值係 **11 個 site**（`s17_triples` 只有 outId 粒度）；**N6 LOW**＝fixture `rel_format.why` 抄咗 R12 reviewer 嘅「unique basename=3」，實測 **6**。
- **v6.14 修法**：① fixture 加 **`s17_sites`（11 條逐 site）＋`s17_site_count`＋`s17_sites_rule`**；② S17 正斷言改 **11 行（逐 site）**、明文唔准用 outId 數；③ `rel_format.why` 更正為「6 unique rel／6 unique basename（R12 誤抄 3，已更正）」。
