...
- **未解**：① C-0／C-1／D 真機未驗（α 已驗 ✓ 見今日 section）② M1e `matched=0` ③ Settings C-2→C-4 ④ token 瘦身要真機 usage ⑤ 副線 7 日 ⑥ 大批 code 未 commit（**跟 SK 定案：等 bump `0.2.2` 一齊 commit＋push**；α 亦一併入呢批）⑦ α defer 12 項（見 plan §8）⑧ **風險已兜**：`OfficialDisplay.java`／`AskDisplayNameCheck.java` untracked，但已有 timestamped 備份（`%TEMP%\packai_alpha_backup_20260916_103725\`，md5 `e14229a5`／`6d02a2cf`）→ 即使 `git clean` 都救得返
