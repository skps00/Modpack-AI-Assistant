...
- **下一步（優先序）**：① **KubeJS 取得通道（code-first）＝ plan `v6.5` 已達 review 閘（正方 8 : 反方 2）→ 等 SK go 派 cursor 實作（單 1）**；SK 09-16 21:0x 明示「選項 2 ＋ until 8:2」，R4 7:3 →（收 N1／N2）→ **R5 8:2 達標**；實作範圍＝新 class `logic/JsObtainSites.java`＋config×2＋`PackIndex` pre-pass＋`ReplyLang`×3＋3 檔 lang（plan §2／§4 有還原點）。② **β = v5.0 API-first（`docs/plans/2026-09-16_kubejs-transform-v5.0-api-first.md` v5.4，commit `d887ac8`）**：R5 正方 8:反方 2＋中立裁判 正方 7 : 反方 3＝**SAFE** → **等 SK go**；⚠️ **與 ① 關係待 SK 拍板**（我判：① 已吸收 β 嘅「JS 副作用」部分＝TRANSFORM 家族 18 站點；β 嘅 recipe 部分已由既有 JEI by-output 覆蓋 → 兩者唔應該同時開工，避免同一功能寫兩次）。③ C-2（answerDetail／blacklist／answerLang）→ C-3 → C-4 ④ bump `0.2.2`＋commit（**push 等 SK**）
...
- **artifact**：plan `docs/plans/2026-09-16_kubejs-obtain-code-channel-v6.5.md`；fixture `docs/plans/fixtures/2026-09-16_nfwc_js_obtain_inventory.json`（**20.9 KB**；`A_hard_gate` 10／`ids_166` 166／`transform_family` 18／`swap_family_excluded` 5／`B_group` 11）。收貨核實：cursor 兩輪 **零 code 改動**（`forge/` 無 21:00 後 mtime）、零可見 console 窗。
