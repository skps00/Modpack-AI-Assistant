...
- **下一步（優先序）**：① **KubeJS 取得通道（code-first）＝ plan `v6.7`（v6.7.2）已達 review 閘（正方 8 : 反方 2）→ 等 SK go 派 cursor 實作（單 1）**；軌跡：R4 7:3 → R5 **8:2** → SK 09-17 糾正（器官 gate／配方交 JEI）→ R6 **4:6** → v6.7 收 N1–N4 → R7 **8:2 達標**。實作範圍＝新 class `logic/JsObtainSites.java`（Site 含 `organGate`／`gateKind`／`heldItem`／`heldSlot`）＋config×2＋`PackIndex` pre-pass＋`ReplyLang`×3＋3 檔 lang；驗收 S1–S15（S12–S15 今日紅）。還原＝單 commit revert＋`jsObtainChannel=false`。**唔准做 KubeJS 配方**（SK 定案：JEI 覆蓋）。② **β = v5.0 API-first（`docs/plans/2026-09-16_kubejs-transform-v5.0-api-first.md` v5.4，commit `d887ac8`）**：R5 正方 8:反方 2＋中立裁判 正方 7 : 反方 3＝**SAFE** → **等 SK go**；⚠️ **與 ① 關係待 SK 拍板**（我判：① 已吸收 β 嘅「JS 副作用」部分＝TRANSFORM 家族 18 站點；β 嘅 recipe 部分已由既有 JEI by-output 覆蓋 → 兩者唔應該同時開工，避免同一功能寫兩次）。③ C-2（answerDetail／blacklist／answerLang）→ C-3 → C-4 ④ bump `0.2.2`＋commit（**push 等 SK**）
