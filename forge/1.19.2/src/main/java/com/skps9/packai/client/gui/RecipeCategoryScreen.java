...
public class RecipeCategoryScreen extends Screen {
    ...
    private void applyFilter() {
        ...
        for (int i = 0; i < this.allRows.size(); i++) {
            JeiCategoryCatalog.Row row = this.allRows.get(i);
            if (query.isEmpty()
    ...
    private void toggleAt(int allIndex) {
        ...
        this.allRows.set(allIndex, row.withEnabled(!row.enabled()));
        persist();
    }
    ...
    private void renderScreen(GuiGraphics graphics, int mouseX, int mouseY, float partialTick) {
        ...
        for (int fi = this.scrollOffset; fi < end; fi++) {
            ...
            if (this.dragFrom >= 0 && this.dragHoverInsert == this.allRows.size() && fi == end - 1) {
