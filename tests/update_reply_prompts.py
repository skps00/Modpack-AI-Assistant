...
EN = {
    ...
    "packai.reply.reply_pattern": (
        ...
        "2. Body: 2–4 plain sentences (materials / steps / purpose)\n"
        "3. One [[recipe:mod:id]] per card the client will show for that item (or {{RECIPE}} / {{RECIPE:n}})\n"
        "Then the next item.\n"
        "\n"
        "Example:\n"
        "[[item:minecraft:iron_pickaxe]] Iron Pickaxe\n"
        "Craft with sticks and iron ingots on a crafting table.\n"
...
ZH_TW = {
    ...
    "packai.reply.reply_pattern": (
        ...
        "2. 正文：2–4 句白話（材料／步驟／用途）\n"
        "3. 該物客戶端會顯示的每張卡各寫一次 [[recipe:mod:id]]（或 {{RECIPE}}／{{RECIPE:n}}）\n"
        "然後下一個物品。\n"
        "\n"
        "範例：\n"
...
ZH_CN = {
    ...
    "packai.reply.reply_pattern": (
        ...
        "2. 正文：2–4 句白话（材料／步骤／用途）\n"
        "3. 该物客户端会显示的每张卡各写一次 [[recipe:mod:id]]（或 {{RECIPE}}／{{RECIPE:n}}）\n"
        "然后下一个物品。\n"
        "\n"
        "范例：\n"
