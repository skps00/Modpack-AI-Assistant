...
EN = {
    "packai.reply.llm_style": (
        ...
        "Never echo prompt section tags starting with [SCROLL_ or [PURPOSE]/[GUIDE]/[VARIANT]/[AS_INGREDIENT]/[CONTAINED]/[TOOL_BUILD]/[TETRA_USE] into the player answer — paraphrase their content in plain chat. Client strips those tags if echoed. Allowed UI markers only: [[item:]] / [[recipe_card:N]] / [[recipe:]] / {{item:}} / {{RECIPE}} per reply pattern.\n"
        "\n"
        "%s\n"
        "Purpose vs as-ingredient:\n"
        "- Purpose = what the item does / how you use it — from purpose/[PURPOSE]/[GUIDE]/[CONSUME_USE]/[TETRA_USE], tooltip, interactions, Patchouli, and Drinkable/Edible/food lines in PURPOSE.\n"
    ...
    "packai.reply.fact_check": (
        ...
        "18. Never echo prompt section tags starting with [SCROLL_ or [PURPOSE]/[GUIDE]/[VARIANT]/[AS_INGREDIENT]/[CONTAINED]/[TOOL_BUILD]/[TETRA_USE] into the player-facing answer. Paraphrase. Client strips them if echoed. Allowed UI markers only per reply pattern ([[item:]] / [[recipe_card:N]] / [[recipe:]] / {{item:}} / {{RECIPE}}).\n"
        "\n"
...
ZH_TW = {
    "packai.reply.llm_style": (
        ...
        "禁止把以 [SCROLL_ 開頭或 [PURPOSE]／[GUIDE]／[VARIANT]／[AS_INGREDIENT]／[CONTAINED]／[TOOL_BUILD]／[TETRA_USE] 的提示區段標籤原樣貼進玩家可見回覆 — 改用白話轉述內容。客戶端若偵測到會剝除。允許的 UI 標記僅限回覆版面：[[item:]]／[[recipe_card:N]]／[[recipe:]]／{{item:}}／{{RECIPE}}。\n"
        "\n"
        "%s\n"
        "用途 vs 作為材料：\n"
        "- 用途＝物品功能／怎麼用 — 依 purpose／[PURPOSE]／[GUIDE]／[CONSUME_USE]／[TETRA_USE]、tooltip、互動、Patchouli，以及 PURPOSE 的 Drinkable／Edible／food 行。\n"
    ...
    "packai.reply.fact_check": (
        ...
        "12. 禁止在 [PURPOSE] 有 Drinkable／Edible／food 行，或 PURPOSE／tooltip 未明說時，自稱「本身沒有直接使用效果／不能用／只是合成或任務材料」— 用途不明時寫本包事實未列用途（勿捏造無效果）。僅有 [AS_INGREDIENT] 不能當無用途證據。有 [TETRA_USE] 時：怎麼用必須寫 Tetra 工作台安裝（零件／插槽／材料／改裝），只轉述區塊的 key／slots／module，禁止捏造未列工具。有此區塊禁止說無用途。\n"
        ...
        "18. 禁止把以 [SCROLL_ 開頭或 [PURPOSE]／[GUIDE]／[VARIANT]／[AS_INGREDIENT]／[CONTAINED]／[TOOL_BUILD]／[TETRA_USE] 的提示區段標籤原樣貼進玩家可見回覆 — 改用白話。客戶端會剝除。允許的 UI 標記僅限回覆版面（[[item:]]／[[recipe_card:N]]／[[recipe:]]／{{item:}}／{{RECIPE}}）。\n"
        "\n"
...
ZH_CN = {
    "packai.reply.llm_style": (
        ...
        "禁止把以 [SCROLL_ 开头或 [PURPOSE]／[GUIDE]／[VARIANT]／[AS_INGREDIENT]／[CONTAINED]／[TOOL_BUILD]／[TETRA_USE] 的提示区段标签原样贴进玩家可见回复 — 改用白话转述内容。客户端若检测到会剥除。允许的 UI 标记仅限回复版面：[[item:]]／[[recipe_card:N]]／[[recipe:]]／{{item:}}／{{RECIPE}}。\n"
        "\n"
        "%s\n"
        "用途 vs 作为材料：\n"
        "- 用途＝物品功能／怎么用 — 依 purpose／[PURPOSE]／[GUIDE]／[CONSUME_USE]／[TETRA_USE]、tooltip、互动、Patchouli，以及 PURPOSE 的 Drinkable／Edible／food 行。\n"
    ...
    "packai.reply.fact_check": (
        ...
        "12. 禁止在 [PURPOSE] 有 Drinkable／Edible／food 行，或 PURPOSE／tooltip 未明说时，自称「本身没有直接使用效果／不能用／只是合成或任务材料」— 用途不明时写本包事实未列用途（勿捏造无效果）。仅有 [AS_INGREDIENT] 不能当无用途证据。有 [TETRA_USE] 时：怎么用必须写 Tetra 工作台安装（零件／插槽／材料／改装），只转述区块的 key／slots／module，禁止捏造未列工具。有此区块禁止说无用途。\n"
        ...
        "18. 禁止把以 [SCROLL_ 开头或 [PURPOSE]／[GUIDE]／[VARIANT]／[AS_INGREDIENT]／[CONTAINED]／[TOOL_BUILD]／[TETRA_USE] 的提示区段标签原样贴进玩家可见回复 — 改用白话。客户端会剥除。允许的 UI 标记仅限回复版面（[[item:]]／[[recipe_card:N]]／[[recipe:]]／{{item:}}／{{RECIPE}}）。\n"
        "\n"
