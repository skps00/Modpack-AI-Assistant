...
- **遇到的問題**：
  - 問題1：model 唔寫 `[card:N]` → 全走 fallback → 全插 section 尾 sticky；uses needles 缺產物名；`subList(0,6)` 冇 diversity
  - 解決方案：R7 Part A/B/C per `cursor_r7_fix.md`
  - 狀態：✅ 已改（NO gradle／NO commit／NO push；shell python checks blocked → 報告列明）
- **備註**：instr `%TEMP%\cursor_r7_fix.md`；report `%TEMP%\cursor_r7_report.md`。
...
[2026-09-20 11:49] F1b inline feature + F1 configuredOwner one-to-many + F2 routeLinesForItem scanModJars gate + worldgen_ore wording. Files: WorldgenFacts.java, WorldgenIndex.java (keepOreRoute), AcquireAskTool.java, JarLightIndex.java, lang en_us/zh_tw/zh_cn, WorldgenRoutesCheck.java, AcquireJarRoutesCheck.java, InfoCompletenessCheck.java, tests/check_worldgen_lookup.py. Tests NOT RUN.
